package cn.hiboot.framework.db.mongo;
...
//                setDefault(listDatabaseName,2,mongoClient);
                ik(listDatabaseName,mongoClient);
            }
        }
    }

    private void ik(String db,MongoClient mongoClient){
        String table = "synonyms";
        for (Document document : mongoClient.getDatabase(db).getCollection(table).listIndexes()) {
            Object o = document.get("name");
            if(o instanceof String){
                String v = (String) o;
                if(v.equals("lowercase_name_1")){
                    mongoClient.getDatabase(db).getCollection(table).dropIndex("lowercase_name_1");
                    mongoClient.getDatabase(db).getCollection(table).createIndex(new Document("lowercase_name","text"));
                    //分词
                    ik2(db, mongoClient,table);
                    return;
                }
            }
        }
    ...
    private void ik2(String db,MongoClient mongoClient,String table){
        IKSeg ikSeg = new IKSeg();

        List<UpdateManyModel<Document>> requests = new ArrayList<>();
        for (Document document : mongoClient.getDatabase(db).getCollection(table).find().projection(new Document("id",1).append("name",1).append("_id",0))) {
            requests.add(new UpdateManyModel<>(new Document("id",document.getLong("id")),new Document("$set",new Document("lowercase_name",ikSeg.parse(document.getString("name"))))));
